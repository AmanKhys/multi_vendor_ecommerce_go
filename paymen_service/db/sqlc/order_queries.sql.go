...
func (q *Queries) GetOrderItemsByUserID(ctx context.Context, userID uuid.UUID) ([]OrderItem, error) {
	rows, err := q.query(ctx, q.getOrderItemsByUserIDStmt, getOrderItemsByUserID, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
